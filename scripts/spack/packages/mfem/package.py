...
class Mfem(BuiltinMfem):
    ...
    depends_on("hipblas", when="@4.7.0.2:+rocm")
...
# TRIBOL_EDIT_BEGIN
            if "^hipblas" in spec:
                ...
                # Note: MFEM's defaults.mk wants to find librocblas.* in
                # $(HIP_DIR)/lib, so we set HIP_DIR to be $ROCM_PATH when using
                # external HIP, or the prefix of rocblas (which is a
                # dependency of hipblas) when using Spack-built HIP.
                if spec["hip"].external:
                    options += ["HIP_DIR=%s" % env["ROCM_PATH"]]
                else:
                    options += ["HIP_DIR=%s" % hipblas["rocblas"].prefix]
# TRIBOL_EDIT_END
