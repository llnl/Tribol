...
void SegmentBasis( const real* const RESTRICT x, const real pX, const real pY, const int numPoints, const int vertexId,
                   real& phi )
{
  SLIC_ERROR_IF( numPoints != 2, "SegmentBasis: numPoints is " << numPoints << " but should be 2." );

  // note, vertexId is the index, 0 or 1.
  SLIC_ERROR_IF( vertexId > numPoints - 1, "SegmentBasis: vertexId is " << vertexId << " but should be 0 or 1." );
...
void InvIso( const real x[3], const real* xA, const real* yA, const real* zA, const int numNodes, real xi[2] )
{
  ...
  for ( int k = 0; k < kmax; ++k ) {
    // evaluate Jacobian
    real j_x_1 = 0.25 * ( xA[0] * ( 1. + x_sol[1] ) - xA[1] * ( 1. + x_sol[1] ) - xA[2] * ( 1. - x_sol[1] ) +
                          xA[3] * ( 1. - x_sol[1] ) );

    real j_y_1 = 0.25 * ( yA[0] * ( 1. + x_sol[1] ) - yA[1] * ( 1. + x_sol[1] ) - yA[2] * ( 1. - x_sol[1] ) +
                          yA[3] * ( 1. - x_sol[1] ) );

    real j_z_1 = 0.25 * ( zA[0] * ( 1. + x_sol[1] ) - zA[1] * ( 1. + x_sol[1] ) - zA[2] * ( 1. - x_sol[1] ) +
                          zA[3] * ( 1. - x_sol[1] ) );

    real j_x_2 = 0.25 * ( xA[0] * ( 1. + x_sol[0] ) + xA[1] * ( 1. - x_sol[0] ) - xA[2] * ( 1. - x_sol[0] ) -
                          xA[3] * ( 1. + x_sol[0] ) );

    real j_y_2 = 0.25 * ( yA[0] * ( 1. + x_sol[0] ) + yA[1] * ( 1. - x_sol[0] ) - yA[2] * ( 1. - x_sol[0] ) -
                          yA[3] * ( 1. + x_sol[0] ) );

    real j_z_2 = 0.25 * ( zA[0] * ( 1. + x_sol[0] ) + zA[1] * ( 1. - x_sol[0] ) - zA[2] * ( 1. - x_sol[0] ) -
                          zA[3] * ( 1. + x_sol[0] ) );
    ...
    if ( abs_dxi_1 <= xtol && abs_dxi_2 <= xtol ) {
      ...
      if ( std::abs( xi[0] ) > 1. || std::abs( xi[0] ) > 1. ) {
        if ( std::abs( xi[0] ) > 1. + 100 * xtol ||
             std::abs( xi[0] ) > 1. + 100 * xtol )  // should have some tolerance dependent conv tol?
        {
          in_quad = false;
        } else {
          xi[0] = std::min( xi[0], 1. );
