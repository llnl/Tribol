...
SubmeshLORTransfer::SubmeshLORTransfer(mfem::ParFiniteElementSpace& submesh_fes, mfem::ParMesh& lor_mesh)
    : lor_gridfn_{CreateLORGridFunction(lor_mesh, std::make_unique<mfem::H1_FECollection>(1, lor_mesh.SpaceDimension()),
                                        submesh_fes.GetVDim())},
...
void SubmeshRedecompTransfer::RedecompToSubmesh(const mfem::GridFunction& redecomp_src, mfem::Vector& submesh_dst) const
{
...
void MfemMeshData::SetLORFactor(int lor_factor)
{
  ...
  lor_mesh_ =
      std::make_unique<mfem::ParMesh>(mfem::ParMesh::MakeRefined(submesh_, lor_factor, mfem::BasisType::ClosedUniform));
...
{
  material_modulus_ =
      std::make_unique<mfem::QuadratureFunction>(new mfem::QuadratureSpace(GetLORMesh() ? GetLORMesh() : &submesh_, 0));
...
void MfemMeshData::UpdateData::UpdateConnectivity(const std::set<int>& attributes_1, const std::set<int>& attributes_2)
{
...
{
  update_data_ = std::make_unique<UpdateData>(*submesh_pressure_.ParFESpace(), submesh_lor_xfer_.get(), redecomp_mesh);
    ...
    : parent_data_{parent_data}, submesh_data_{submesh_data}, block_offsets_{3}
{
  ...
  auto dof_offset = parent_data_.GetParentCoords().ParFESpace()->GetDofOffsets()[offset_idx];
...
std::unique_ptr<mfem::BlockOperator> MfemJacobianData::GetMfemBlockJacobian(const MethodData* method_data) const
{
  ...
  auto submesh_J = GetUpdateData().submesh_redecomp_xfer_->TransferToParallelSparse(lm_elems, mortar_elems, *elem_J_1);
  submesh_J += GetUpdateData().submesh_redecomp_xfer_->TransferToParallelSparse(lm_elems, nonmortar_elems, *elem_J_2);
  submesh_J.Finalize();
  ...
  auto J_full = std::make_unique<mfem::HypreParMatrix>(mpi.MPIComm(), submesh_fes.GetVSize(), submesh_fes.GlobalVSize(),
                                                       parent_trial_fes.GlobalVSize(), submesh_J.GetI(),
                                                       submesh_J.GetJ(), submesh_J.GetData(),
...
MfemJacobianData::UpdateData::UpdateData(const MfemMeshData& parent_data, const MfemSubmeshData& submesh_data)
{
  ...
  // submesh
  submesh_redecomp_xfer_ = std::make_unique<redecomp::MatrixTransfer>(*dual_submesh_fes, *primal_submesh_fes,
