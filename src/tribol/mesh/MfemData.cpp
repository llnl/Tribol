                                                 ...
                                                 mfem::Vector& submesh_dst ) const
{
  ...
  auto* P_hypre = dst_fespace_ptr->Dof_TrueDof_Matrix();
  SLIC_ERROR_ROOT_IF( P_hypre == nullptr, "Null Dof_TrueDof_Matrix()." );
  mfem::SparseMatrix P_diag;
  P_hypre->GetDiag( P_diag );
  const int* P_I = P_diag.GetI();
  SLIC_ERROR_ROOT_IF( P_I == nullptr, "Invalid diag block in Dof_TrueDof_Matrix()." );
  HYPRE_Int tdof_ct{ 0 };
