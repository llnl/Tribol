...
void MfemMeshData::UpdateMfemMeshData( RealT binning_proximity_scale, int n_ranks )
{
  ...
  update_data_ = std::make_unique<UpdateData>( submesh_, lor_mesh_.get(), *coords_.GetParentGridFn().ParFESpace(),
                                               ...
                                               attributes_2_, binning_proximity_scale, n_ranks );
...
MfemMeshData::UpdateData::UpdateData( mfem::ParSubMesh& submesh, mfem::ParMesh* lor_mesh,
                                      ...
                                      RealT binning_proximity_scale, int n_ranks )
...
std::unique_ptr<mfem::BlockOperator> MfemJacobianData::GetMfemBlockJacobian( const MethodData* method_data ) const
{
  ...
  auto submesh_J =
      GetUpdateData().submesh_redecomp_xfer_->TransferToParallelSparse( lm_elems, mortar_elems, *elem_J_1 );
