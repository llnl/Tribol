...
int ApplySmoothNormalEnzyme( CouplingScheme* cs )
{
  ...
  if ( lm_opts.eval_mode == ImplicitEvalMode::MORTAR_RESIDUAL_JACOBIAN ||
    ...
    if ( lm_opts.sparse_mode == SparseMode::MFEM_ELEMENT_DENSE ) {
      cs->getMethodData()->reserveBlockJ( { BlockSpace::NONMORTAR, BlockSpace::MORTAR }, planes_view.size() );
  ...
  for ( auto& plane : planes_view ) {
    ...
    if ( lm_opts.eval_mode == ImplicitEvalMode::MORTAR_RESIDUAL_JACOBIAN ||
      ...
      RealT lenA = sqrt( ( coords[2] - coords[0] ) * ( coords[2] - coords[0] ) +
                         ( coords[3] - coords[1] ) * ( coords[3] - coords[1] ) );
      RealT lenB = sqrt( ( coords[6] - coords[4] ) * ( coords[6] - coords[4] ) +
                         ( coords[7] - coords[5] ) * ( coords[7] - coords[5] ) );
...
void find_normal( const RealT* coord1, const RealT* coord2, RealT* normal )
{
  ...
  RealT len = 1.0 / std::sqrt( dy * dy + dx * dx );
  dx *= len;
  dy *= len;
  normal[0] = dy;
  normal[1] = -dx;
...
bool segmentsIntersect( const RealT A0[2], const RealT A1[2], const RealT B0[2], const RealT B1[2],
                        RealT intersection[2] )
{
  ...
  RealT denom = dxA * dyB - dyA * dxB;
  RealT numeA = dxAB * dyB - dyAB * dxB;
  RealT numeB = dxAB * dyA - dyAB * dxA;

  // Collinear or parallel (it could intersect at a single point, but we don't care)
...
void get_projections( const RealT* A0, const RealT* A1, const RealT* B0, const RealT* B1, RealT* projections,
                      RealT del )
{
  RealT nA[2] = { 0.0 };
  RealT nB[2] = { 0.0 };
...
void modify_bounds( const RealT* integration_bounds, RealT del, RealT* modified_bounds )
{
  ...
  for ( int i = 0; i < 2; ++i ) {
    ...
    if ( 0.0 - del <= xi && xi <= del ) {
      ...
      RealT d = 1.0 - del + ( 1.0 / ( 4.0 * del ) ) * pow( 1.0 - del, 2 ) - 0.5 * ( 1.0 - del ) -
                ( 1.0 - del ) / ( 2.0 * del );
...
void compute_quadrature_point( const RealT* integration_bounds, const RealT* A0, const RealT* A1, int N,
                               RealT* quad_points )
{
  RealT eta_values[N];
...
void ComputeSmoothMortarEnergyEnzyme( const RealT* coords, RealT del, RealT k1, RealT k2, int N, RealT lenA, RealT lenB,
                                      RealT* energy )
{
  ...
  RealT nA[2] = { 0.0 };
  RealT nB[2] = { 0.0 };
  // NOTE: work with the original coords on A instead of A0 and A1, otherwise enzyme won't compile with release.  result
  // will be the same and the derivatives are a bit simpler.
  find_normal( coords, coords + 2, nA );
  // NOTE: do the same on B, to be consistent.
  find_normal( coords + 4, coords + 6, nB );
  ...
  if ( std::abs( dot_product ) < 1e-10 ) {
    ...
    *energy = 0.0;
    ...
    // lenA = sqrt( ( coords[2] - coords[0] ) * ( coords[2] - coords[0] ) +
    //              ( coords[3] - coords[1] ) * ( coords[3] - coords[1] ) );
    *energy *= lenA * 0.5;
...
// void ComputeSmoothMortarEnergyEnzyme( const RealT* x1, const RealT* n1, const RealT* p1, RealT* f1, RealT* g1, int
// size1,
...
void ComputeSmoothMortarForceEnzyme( RealT* coords, RealT del, RealT k1, RealT k2, int N, RealT lenA, RealT lenB,
                                     RealT* dE_dX )
{
  double dcoords[8] = { 0.0 };
  double E = 0.0;
  double dE = 1.0;
  __enzyme_autodiff<void>( ComputeSmoothMortarEnergyEnzyme, enzyme_dup, coords, dcoords, enzyme_const, del,
                           enzyme_const, k1, enzyme_const, k2, enzyme_const, N, enzyme_const, lenA, enzyme_const, lenB,
                           enzyme_dup, &E, &dE );
...
// void ComputeSmoothMortarForceEnzyme( const RealT* x1, const RealT* n1, const RealT* p1, RealT* f1, RealT* g1, int
// size1,
//                                      const RealT* x2, RealT* f2, int size2 )
// {
...
void ComputeSmoothMortarJacobianEnzyme( RealT* coords, RealT del, RealT k1, RealT k2, int N, RealT lenA, RealT lenB,
                                        RealT* force, RealT* d2E_d2X )
{
  RealT dE[8] = { 0.0 };
  RealT d2E[8] = { 0.0 };
  RealT dEf[8] = { 0.0 };
  ...
  for ( int i = 0; i < 8; ++i ) {
    ...
    RealT d2lenA = 0.0;
    RealT d2lenB = 0.0;
    __enzyme_fwddiff<void>( ComputeSmoothMortarForceEnzyme, coords, d2coords, del, d2del, k1, d2k1, k2, d2k2, N, lenA,
                            d2lenA, lenB, d2lenB, &dE, &d2E );
    for ( int j = 0; j < 8; ++j ) {
      d2E_d2X[8 * i + j] = d2E[j];
...
//                                         RealT* df1dx2, RealT* df1dn1, RealT* df1dp1, RealT* g1, RealT* dg1dx1, RealT*
//                                         dg1dx2, RealT* dg1dn1, int size1, const RealT* x2, RealT* f2, RealT* df2dx1,
//                                         RealT* df2dx2, RealT* df2dn1, RealT* df2dp1, int size2 )
// {
// }
#endif
