...
void ComputeConstraintJacobian<SINGLE_MORTAR, PRIMAL>(SurfaceContactElem& elem)
{
  ...
  for (int a = 0; a < elem.numFaceVert; ++a) {
    ...
    for (int b = 0; b < elem.numFaceVert; ++b) {
      ...
      elem.blockJ(static_cast<IndexT>(BlockSpace::LAGRANGE_MULTIPLIER),
                  static_cast<IndexT>(BlockSpace::NONMORTAR))[elem_xdof + 2 * dim_offset] -= nrml_a[2] * n_nonmortar_a;
...
int ApplyNormalEnzyme(CouplingScheme* cs)
{
  ...
  if (lm_opts.eval_mode == ImplicitEvalMode::MORTAR_RESIDUAL_JACOBIAN ||
    ...
    if (lm_opts.sparse_mode == SparseMode::MFEM_ELEMENT_DENSE) {
      cs->getMethodData()->reserveBlockJ({BlockSpace::NONMORTAR, BlockSpace::MORTAR, BlockSpace::LAGRANGE_MULTIPLIER},
                                         planes_view.size());
      cs->createNormalJacobian();
      cs->getdnMethodData()->reserveBlockJ({BlockSpace::NONMORTAR, BlockSpace::MORTAR, BlockSpace::LAGRANGE_MULTIPLIER},
                                           planes_view.size());
  ...
  for (auto& plane : planes_view) {
    ...
    if (lm_opts.eval_mode == ImplicitEvalMode::MORTAR_RESIDUAL_JACOBIAN ||
      ...
      ComputeMortarJacobianEnzyme(x1, n1, p1, f1, blockJ(0, 0).data(), blockJ(0, 1).data(), blockJ_n(0, 0).data(),
                                  blockJ(0, 2).data(), g1, blockJ(2, 0).data(), blockJ(2, 1).data(),
                                  blockJ_n(2, 0).data(), size1, x2, f2, blockJ(1, 0).data(), blockJ(1, 1).data(),
                                  blockJ_n(1, 0).data(), blockJ(1, 2).data(), size2);
