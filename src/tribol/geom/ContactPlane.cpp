...
//------------------------------------------------------------------------------
TRIBOL_HOST_DEVICE bool FaceInterCheck(const MeshData::Viewer& mesh1, const MeshData::Viewer& mesh2, int fId1, int fId2,
                                       RealT tol, bool& allVerts)
{
  bool check = false;
  allVerts = false;
...
//------------------------------------------------------------------------------
TRIBOL_HOST_DEVICE bool EdgeInterCheck(const MeshData::Viewer& mesh1, const MeshData::Viewer& mesh2, int eId1, int eId2,
                                       RealT tol, bool& allVerts)
{
  bool check = false;
  allVerts = false;
                                               ...
                                               const RealT gap)
{
  if (params.auto_contact_check) {
    RealT max_interpen =
        -1. * params.auto_contact_pen_frac *
        axom::utilities::min(mesh1.getElementData().m_thickness[faceId1], mesh2.getElementData().m_thickness[faceId2]);
    if (gap < max_interpen) {
      return true;
    }
  }
                                               ...
                                               bool fullOverlap)
{
  ...
  if (fullOverlap) {
    ...
    FaceGeomError inter_err =
        Intersection2DPolygon(X1, Y1, mesh1.numberOfNodesPerElement(), X2, Y2, mesh2.numberOfNodesPerElement(), pos_tol,
                              len_tol, cp.m_polyLocX, cp.m_polyLocY, cp.m_numPolyVert, cp.m_area, false);
  ...
  // needed.
  cp.planePointAndCentroidGap(
      mesh1, mesh2, 2. * axom::utilities::max(mesh1.getFaceRadius()[element_id1], mesh2.getFaceRadius()[element_id2]));
                                                       ...
                                                       const MeshData::Viewer& mesh2, const Parameters& params)
{
  ...
  cp.m_areaMin =
      cp.m_areaFrac * axom::utilities::min(mesh1.getElementAreas()[element_id1], mesh2.getElementAreas()[element_id2]);
...
TRIBOL_HOST_DEVICE void ContactPlane3D::computeLocalBasis(const MeshData::Viewer& m1)
{
  ...
  // project to plane
  RealT pX, pY, pZ;
  ProjectPointToPlane(m1.getPosition()[0][nodeId], m1.getPosition()[1][nodeId], m1.getPosition()[2][nodeId], m_nX, m_nY,
                      m_nZ, m_cX, m_cY, m_cZ, pX, pY, pZ);
                                                                             ...
                                                                             const Parameters& params, bool& interpen)
{
  ...
  RealT len_tol = pos_tol;
  FaceGeomError inter_err = Intersection2DPolygon(cfx1_loc, cfy1_loc, numV[0], cfx2_loc, cfy2_loc, numV[1], pos_tol,
                                                  len_tol, m_polyLocX, m_polyLocY, m_numPolyVert, m_interpenArea, true);
