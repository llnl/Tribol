                                ...
                                EnforcementMethod enforcement_method, BinningMethod binning_method)
{
  ...
  auto mfem_data = std::make_unique<MfemMeshData>(mesh_id_1, mesh_id_2, mesh, current_coords, std::move(b_attributes_1),
  ...
  if (enforcement_method == LAGRANGE_MULTIPLIER) {
    std::unique_ptr<mfem::FiniteElementCollection> pressure_fec =
        std::make_unique<mfem::H1_FECollection>(current_coords.FESpace()->FEColl()->GetOrder(), mesh.SpaceDimension());
    int pressure_vdim = 0;
    ...
    if (coupling_scheme.hasMfemData()) {
      ...
      if (penalty_opts.kinematic_calc_set) {
        if (penalty_opts.kinematic_calculation == KINEMATIC_ELEMENT) {
          ...
          SLIC_ERROR_ROOT_IF(!mfem_data->GetRedecompMaterialModulus1() || !mfem_data->GetRedecompMaterialModulus2(),
...
void saveRedecompMesh(int output_id)
{
  for (auto& cs_pair : CouplingSchemeManager::getInstance()) {
    ...
    if (coupling_scheme.hasMfemData()) {
      ...
      std::string dc_name("redecomp_cs" + std::to_string(cs_pair.first) + "_id" + std::to_string(output_id) + "_rank" +
