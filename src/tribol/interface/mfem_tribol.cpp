...
void registerMfemCouplingScheme( integer cs_id, integer mesh_id_1, integer mesh_id_2, const mfem::ParMesh& mesh,
                                 const mfem::ParGridFunction& current_coords, std::set<integer> b_attributes_1,
                                 std::set<integer> b_attributes_2, ContactMode contact_mode, ContactCase contact_case,
                                 ...
                                 EnforcementMethod enforcement_method, BinningMethod binning_method )
{
  ...
  registerMesh( mesh_id_1, 0, 0, nullptr, 1, nullptr, nullptr );
  registerMesh( mesh_id_2, 0, 0, nullptr, 1, nullptr, nullptr );
  registerCouplingScheme( cs_id, mesh_id_1, mesh_id_2, contact_mode, contact_case, contact_method, contact_model,
                          enforcement_method, binning_method );
  ...
  if ( enforcement_method == LAGRANGE_MULTIPLIER ) {
    ...
    coupling_scheme->setMfemSubmeshData( std::make_unique<MfemSubmeshData>(
        mfem_data->GetSubmesh(), mfem_data->GetLORMesh(), std::move( pressure_fec ), pressure_vdim ) );
    ...
    if ( lm_options.enforcement_option_set && ( lm_options.eval_mode == ImplicitEvalMode::MORTAR_JACOBIAN ||
      ...
      coupling_scheme->setMfemJacobianData(
          std::make_unique<MfemJacobianData>( *mfem_data, *coupling_scheme->getMfemSubmeshData(), contact_method ) );
    }
  }
...
{
  CouplingScheme* coupling_scheme = CouplingSchemeManager::getInstance().getCoupling( csId );
  SparseMode sparse_mode = coupling_scheme->getEnforcementOptions().lm_implicit_options.sparse_mode;
  ...
  return coupling_scheme->getMfemJacobianData()->GetMfemBlockJacobian( coupling_scheme->getMethodData() );
}

void getMfemGap( integer cs_id, mfem::Vector& g )
{
  auto coupling_scheme = CouplingSchemeManager::getInstance().getCoupling( cs_id );
  SLIC_ERROR_ROOT_IF( !coupling_scheme->hasMfemSubmeshData(),
...
void saveRedecompMesh( integer output_id )
{
  ...
  for ( int csIndex = 0; csIndex < numCouplings; ++csIndex ) {
    ...
    if ( couplingScheme->hasMfemData() ) {
      ...
      std::string dc_name( "redecomp_cs" + std::to_string( csIndex ) + "_id" + std::to_string( output_id ) + "_rank" +
