...
/// @name Owning MFEM ENERGY_MORTAR evaluation
///
/// These functions return owning true-DOF vectors and Hypre matrices. The
/// overloads without an IntegrationRule synchronize MFEM fields, rebuild
/// redecomp state when needed, run search, and build a new rule. Rule-taking
/// overloads reuse the rule's geometry snapshot without those preparation steps.
/// @{

/// Build and return an owning integration rule for an MFEM ENERGY_MORTAR scheme.
...
/// Copy a true-DOF pressure or Lagrange-multiplier vector into the scheme FieldData.
