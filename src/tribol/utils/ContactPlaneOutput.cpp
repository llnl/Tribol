...
void WriteContactPlaneMeshToVtk(const std::string& dir, const VisType v_type, const IndexT cs_id, const IndexT mesh_id1,
                                const IndexT mesh_id2, const int dim, const int cycle, const RealT time)
{
  ...
  if (!couplingScheme->nullMeshes()) {
    ...
    if (faces && cpSize > 0) {
      ...
      for (int i = 0; i < cpSize; ++i) {
        auto& cp = couplingScheme->getContactPlane(i);
        // print face areas
        axom::fmt::print(faces, "{} {} ", mesh1.getElementAreas()[cp.getCpElementId1()],
    ...
    //////////////////////////////////////////////////////////////
    if (meshes) {
      std::string name = (nranks > 1) ? axom::fmt::format("mesh_intrfc_cs{:02}_r{:04}_{:07}.vtk", cs_id, rank, cycle)
