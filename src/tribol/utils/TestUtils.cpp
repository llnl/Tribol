...
//------------------------------------------------------------------------------
void TestMesh::setupContactMeshHex(int numElemsX1, int numElemsY1, int numElemsZ1, RealT xMin1, RealT yMin1,
                                   RealT zMin1, RealT xMax1, RealT yMax1, RealT zMax1, int numElemsX2, int numElemsY2,
                                   int numElemsZ2, RealT xMin2, RealT yMin2, RealT zMin2, RealT xMax2, RealT yMax2,
                                   RealT zMax2, RealT thetaMortar, RealT thetaNonmortar)
{
...
//------------------------------------------------------------------------------
void TestMesh::setupContactMeshTet(int numElemsX1, int numElemsY1, int numElemsZ1, RealT xMin1, RealT yMin1,
                                   RealT zMin1, RealT xMax1, RealT yMax1, RealT zMax1, int numElemsX2, int numElemsY2,
                                   int numElemsZ2, RealT xMin2, RealT yMin2, RealT zMin2, RealT xMax2, RealT yMax2,
                                   RealT zMax2, RealT thetaMortar, RealT thetaNonmortar)
{
  ...
  setupContactMeshHex(numElemsX1, numElemsY1, numElemsZ1, xMin1, yMin1, zMin1, xMax1, yMax1, zMax1, numElemsX2,
                      numElemsY2, numElemsZ2, xMin2, yMin2, zMin2, xMax2, yMax2, zMax2, thetaMortar, thetaNonmortar);
...
//------------------------------------------------------------------------------
int TestMesh::simpleTribolSetupAndUpdate(ContactMethod method, EnforcementMethod TRIBOL_UNUSED_PARAM(enforcement),
                                         ContactModel TRIBOL_UNUSED_PARAM(model),
                                         ContactCase TRIBOL_UNUSED_PARAM(contact_case),
                                         bool TRIBOL_UNUSED_PARAM(visualization), TestControlParameters& params)
{
  ...
  SimpleCouplingSetup(this->dim, this->cellType, method, this->numMortarFaces, this->numTotalNodes, this->faceConn1, x,
                      y, z, this->numNonmortarFaces, this->numTotalNodes, this->faceConn2, x, y, z, area_frac,
                      this->gaps, this->pressures);
...
//------------------------------------------------------------------------------
int TestMesh::tribolSetupAndUpdate(ContactMethod method, EnforcementMethod enforcement, ContactModel model,
                                   ContactCase contact_case, bool visualization, TestControlParameters& params)
{
  // grab coordinate data
  RealT* x = this->x;
  RealT* y = this->y;
  ...
  registerMesh(this->mortarMeshId, this->numMortarFaces, this->numTotalNodes, this->faceConn1, this->cellType, x, y, z,
               MemorySpace::Host);
  registerMesh(this->nonmortarMeshId, this->numNonmortarFaces, this->numTotalNodes, this->faceConn2, this->cellType, x,
               y, z, MemorySpace::Host);
  ...
  registerNodalResponse(this->mortarMeshId, this->fx1, this->fy1, this->fz1);

  registerNodalResponse(this->nonmortarMeshId, this->fx2, this->fy2, this->fz2);

  // register nodal velocities
  if (registered_velocities1) {
    registerNodalVelocities(this->mortarMeshId, this->vx1, this->vy1, this->vz1);
  }
  ...
  if (enforcement == PENALTY) {
    if (!params.penalty_ratio) {
      ...
      registerRealElementField(this->mortarMeshId, BULK_MODULUS, this->mortar_bulk_mod);
      registerRealElementField(this->mortarMeshId, ELEMENT_THICKNESS, this->mortar_element_thickness);

      // register nonmortar penalty data
      registerRealElementField(this->nonmortarMeshId, BULK_MODULUS, this->nonmortar_bulk_mod);
      registerRealElementField(this->nonmortarMeshId, ELEMENT_THICKNESS, this->nonmortar_element_thickness);
  ...
  const int csIndex = 0;
  registerCouplingScheme(csIndex, this->mortarMeshId, this->nonmortarMeshId, SURFACE_TO_SURFACE, contact_case, method,
  ...
  if (method == COMMON_PLANE && enforcement == PENALTY) {
    PenaltyConstraintType constraint_type =
        (params.constant_rate_penalty || params.percent_rate_penalty) ? KINEMATIC_AND_RATE : KINEMATIC;
    ...
    setLagrangeMultiplierOptions(csIndex, ImplicitEvalMode::MORTAR_RESIDUAL_JACOBIAN, SparseMode::MFEM_LINKED_LIST);
  } else if (method == MORTAR_WEIGHTS) {
    // note, eval modes and sparse modes not exposed in the interface to this class
    setLagrangeMultiplierOptions(csIndex, ImplicitEvalMode::MORTAR_WEIGHTS_EVAL, SparseMode::MFEM_LINKED_LIST);
  }
...
//------------------------------------------------------------------------------
void TestMesh::setupPatchTestDirichletBCs(IndexT mesh_id, int numElemsX, int numElemsY, int numElemsZ, int nodeIdOffset,
                                          bool inHomogeneousGap, RealT inHomogeneousZVal)
{
...
//------------------------------------------------------------------------------
void TestMesh::setupPatchTestPressureDofs(IndexT mesh_id, int numElemsX, int numElemsY, int numElemsZ, int nodeIdOffset,
                                          bool contact)
{
...
void TestMesh::setupMfemMesh(bool fix_orientation)
{
  ...
  this->mfem_mesh = new mfem::Mesh(this->dim, this->numTotalNodes, this->numTotalElements);
...
//------------------------------------------------------------------------------
void TestMesh::computeEquilibriumJacobian(mfem::SparseMatrix* A, RealT const nu, RealT const youngs)
{
  SLIC_ERROR_IF(this->mfem_mesh == nullptr, "TestMesh::computeEquilibriumJacobian(): must call setupMfemMesh() "
...
//------------------------------------------------------------------------------
void TestMesh::computeEquilibriumJacobian(mfem::SparseMatrix* const A, mfem::ElasticityIntegrator* eInteg,
...
void TestMesh::tribolMatrixToSystemMatrix(mfem::DenseMatrix* const ATribol, mfem::SparseMatrix* const ASystem)
{
  ...
  for (int i = 0; i < this->numNonmortarSurfaceNodes; ++i) {
    ...
    int oldOffset = this->dim * this->numTotalNodes + this->numMortarNodes;
...
void TestMesh::getGapEvals(RealT* const v)
{
  ...
  for (int a = 0; a < this->numNonmortarNodes; ++a) {
    ...
    {
      v[presOffset + presDofCtr] = -this->gaps[this->numMortarNodes + a];  // we have negative rhs
      ++presDofCtr;
    }
  }
    ...
    for (int m = 0; m < numBlkNodes; ++m) {
      ...
      if (!contact) {
        if (presDofs[m] >= 0) {
          ...
          int testPresRowId = this->dim * this->numTotalNodes + presCtr;
...
void TestMesh::testMeshToVtk(const std::string& dir, int cycle, RealT time)
{
  ...
  std::string f_name = axom::utilities::filesystem::joinPath(dir, "meshTest3D_" + suffix_mesh.str());
  ...
  for (int i = 0; i < this->numTotalNodes; ++i) {
    mesh << this->x[i] << " " << this->y[i] << " " << this->z[i] << std::endl;
  }
...
void ExplicitMechanics::Mult(const mfem::Vector& u, const mfem::Vector& AXOM_UNUSED_PARAM(dudt), mfem::Vector& a) const
{
