...
TEST_F(EnzymePatchTest, FiniteDiffCheckShifted2x2Meshes)
{
  ...
  // create mesh0
  auto mesh0 = mfem::Mesh::MakeCartesian3D(num_xel_mesh0, num_yel_mesh0, 1, mfem::Element::HEXAHEDRON,
    el_width, el_width, el_height);
  ...
  // create mesh1
  auto mesh1 = mfem::Mesh::MakeCartesian3D(num_xel_mesh1, num_yel_mesh1, 1, mfem::Element::HEXAHEDRON,
    el_width, el_width, el_height);
  ...
  // create MFEM submesh
  mfem::Array<int> submesh_bdry_attribs({mesh0_bdry_attrib, mesh1_bdry_attrib});
  ...
  constexpr auto num_contact_els_mesh0 = num_xel_mesh0 * num_yel_mesh0;
  ...
  {
    if (submesh.GetAttribute(e) == mesh0_bdry_attrib)
    {
  ...
  constexpr auto num_contact_els_mesh1 = num_xel_mesh1 * num_yel_mesh1;
  ArrayT<IndexT> mesh1_elem_map(0, num_contact_els_mesh1);
  ArrayT<IndexT, 2> mesh1_conn(num_contact_els_mesh1, 4);
  for (int e{0}; e < submesh.GetNE(); ++e)
  {
    if (submesh.GetAttribute(e) == mesh1_bdry_attrib)
    {
      mfem::Array<int> elem_dofs(4);
      fe_space.GetElementDofs(e, elem_dofs);
      for (int i{0}; i < 4; ++i)
  ...
  registerMesh(mesh0_id, num_contact_els_mesh0, fe_space.GetNDofs(), mesh0_conn.data(),
    LINEAR_QUAD, coords_ptr, coords_ptr + fe_space.GetNDofs(),
    coords_ptr + 2*fe_space.GetNDofs());
  constexpr auto mesh1_id = 1;
  registerMesh(mesh1_id, num_contact_els_mesh1, fe_space.GetNDofs(), mesh1_conn.data(),
    LINEAR_QUAD, coords_ptr, coords_ptr + fe_space.GetNDofs(),
    coords_ptr + 2*fe_space.GetNDofs());
  ...
  registerNodalReferenceCoords(mesh0_id, 
    ref_coords_ptr, ref_coords_ptr + fe_space.GetNDofs(), ref_coords_ptr + 2*fe_space.GetNDofs());
  registerNodalReferenceCoords(mesh1_id, 
    ref_coords_ptr, ref_coords_ptr + fe_space.GetNDofs(), ref_coords_ptr + 2*fe_space.GetNDofs());
  ...
  registerNodalResponse(mesh0_id, 
    force_ptr, force_ptr + fe_space.GetNDofs(), force_ptr + 2*fe_space.GetNDofs());
  registerNodalResponse(mesh1_id, 
    force_ptr, force_ptr + fe_space.GetNDofs(), force_ptr + 2*fe_space.GetNDofs());
  ...
  registerCouplingScheme(cs_id, mesh0_id, mesh1_id,
  ...
  auto elem_Js = &dfdx_data.getBlockJ()(
  ...
  dndx_file.close();
  auto dfdn_dndx = std::unique_ptr<mfem::SparseMatrix>(mfem::Mult(
    dfdn, dndx
  ));
  auto dfdx = std::unique_ptr<mfem::SparseMatrix>(mfem::Add(
    dfdx_nc, *dfdn_dndx
  ));
  ...
  dgdn.Finalize();
  auto dgdn_dndx = std::unique_ptr<mfem::SparseMatrix>(mfem::Mult(
    dgdn, dndx
  ));
  auto dgdx = std::unique_ptr<mfem::SparseMatrix>(mfem::Add(
    dgdx_nc, *dgdn_dndx
  ));
  ...
  auto delta = 1.0e-7;
  setLagrangeMultiplierOptions(cs_id, 
    ImplicitEvalMode::MORTAR_RESIDUAL, SparseMode::MFEM_ELEMENT_DENSE);
  ...
  for (int i{0}; i < fe_space.GetVSize(); ++i)
  {
    ...
    {
      dfdx_fd(j, i) = (force[j] - force_base[j]) / delta;
    }
    for (int j{0}; j < fe_space_scalar.GetVSize(); ++j)
    {
      dgdx_fd(j, i) = (gap[j] - gap_base[j]) / delta;
    }
    coords[i] -= delta;
  }
  ...
  for (int i{0}; i < fe_space.GetVSize(); ++i)
  {
    for (int j{0}; j < fe_space.GetVSize(); ++j)
    {
      ...
      {
        std::cout << "  (" << i << ", " << j << ") : Diff: " << 
          diff << "   Ratio: " << dfdx_enzyme(i,j) / dfdx_fd(i,j) << "   Enzyme: " <<
          dfdx_enzyme(i,j) << "   FD: " << dfdx_fd(i,j) << std::endl;
      }
  ...
  for (int i{0}; i < fe_space_scalar.GetVSize(); ++i)
  {
    for (int j{0}; j < fe_space.GetVSize(); ++j)
    {
      ...
      if (diff > delta)
      {
        std::cout << "  (" << i << ", " << j << ") : Diff: " << 
          diff << "   Enzyme: " <<
          dgdx_enzyme(i,j) << "   FD: " << dgdx_fd(i,j) << std::endl;
      }
  ...
  for (int i{0}; i < fe_space.GetVSize(); ++i)
  {
    for (int j{0}; j < fe_space_scalar.GetVSize(); ++j)
    {
      ...
      if (diff > delta)
      {
        std::cout << "  (" << i << ", " << j << ") : Diff: " << 
          diff << "   Enzyme: " <<
          dfdp_enzyme(i,j) << "   FD: " << dfdp_fd(i,j) << std::endl;
      }
