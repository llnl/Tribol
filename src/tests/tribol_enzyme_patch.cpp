...
class EnzymePatchTest : public testing::Test {
 protected:
                    ...
                    const double* stencil_dir = nullptr)
  {
    ...
    auto elem_Js = &dfdx_data.getBlockJ()(static_cast<int>(BlockSpace::MORTAR), static_cast<int>(BlockSpace::MORTAR));
    ...
    elem_Js = &dfdx_data.getBlockJ()(static_cast<int>(BlockSpace::MORTAR), static_cast<int>(BlockSpace::NONMORTAR));
    for (int i{0}; i < num_pairs; ++i) {
    ...
    elem_Js = &dfdx_data.getBlockJ()(static_cast<int>(BlockSpace::NONMORTAR), static_cast<int>(BlockSpace::MORTAR));
    for (int i{0}; i < num_pairs; ++i) {
      auto elem_row_dofs = mfem::Array<int>();
    ...
    elem_Js = &dfdx_data.getBlockJ()(static_cast<int>(BlockSpace::NONMORTAR), static_cast<int>(BlockSpace::NONMORTAR));
    for (int i{0}; i < num_pairs; ++i) {
      auto elem_dofs = mfem::Array<int>();
    ...
    // get mortar/nonmortar contributions
    elem_Js = &dfdn_data.getBlockJ()(static_cast<int>(BlockSpace::MORTAR), static_cast<int>(BlockSpace::NONMORTAR));
    for (int i{0}; i < num_pairs; ++i) {
      auto elem_row_dofs = mfem::Array<int>();
      fe_space.GetElementVDofs(mortar_elems[i], elem_row_dofs);
      auto elem_col_dofs = mfem::Array<int>();
    ...
    // get nonmortar/nonmortar contributions
    elem_Js = &dfdn_data.getBlockJ()(static_cast<int>(BlockSpace::NONMORTAR), static_cast<int>(BlockSpace::NONMORTAR));
    for (int i{0}; i < num_pairs; ++i) {
      auto elem_dofs = mfem::Array<int>();
      fe_space.GetElementVDofs(nonmortar_elems[i], elem_dofs);
      for (int j{0}; j < elem_dofs.Size(); ++j) {
    ...
    // get nonmortar/nonmortar contributions
    elem_Js = &dndx_data.getBlockJ()(static_cast<int>(BlockSpace::NONMORTAR), static_cast<int>(BlockSpace::NONMORTAR));
    ...
    elem_Js =
        &dfdx_data.getBlockJ()(static_cast<int>(BlockSpace::LAGRANGE_MULTIPLIER), static_cast<int>(BlockSpace::MORTAR));
    for (int i{0}; i < num_pairs; ++i) {
      auto elem_row_dofs = mfem::Array<int>();
    ...
    elem_Js =
        &dfdx_data.getBlockJ()(static_cast<int>(BlockSpace::MORTAR), static_cast<int>(BlockSpace::LAGRANGE_MULTIPLIER));
    for (int i{0}; i < num_pairs; ++i) {
      auto elem_row_dofs = mfem::Array<int>();
      fe_space.GetElementVDofs(mortar_elems[i], elem_row_dofs);
      auto elem_col_dofs = mfem::Array<int>();
